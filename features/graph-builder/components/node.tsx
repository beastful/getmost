...
import { NODES, substituteInputs } from "@/features/graph-builder/lib/data/nodes";
...
export const FlowNode = ({ id, data, selected }: NodeProps) => {
  ...
  // Stabilise reference so effects don't spin on every store mutation
  const inputs = useMemo(() => rawInputs, [JSON.stringify(rawInputs)]);

  // ── Templates: unified template field ──
  //   function  → called with (inputs, state)
  //   array     → substituteInputs() (binds ["input", "id", fallback])
  //   primitive → passed through as-is
  const templates = useMemo(() => {
    ...
    for (const output of nodeDef.outputs) {
      ...
      if (typeof val === "function") {
        ...
        val = substituteInputs(val, inputs);
      }
